...
import { ArrowRight, Check, CheckCircle2, Copy, ExternalLink, ShieldCheck, Ticket, Wallet, WifiOff } from 'lucide-react';
...
const inputClass = 'mt-2 min-h-14 w-full rounded-2xl border border-input bg-background/80 px-4 py-3 text-base text-foreground shadow-inner shadow-foreground/5 placeholder:text-muted-foreground focus-visible:border-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:opacity-60 motion-safe:transition-colors';
...
function Address({ value, label = 'Wallet' }: { value: string; label?: string }) {
  ...
  return <div className="min-w-0"><span className="text-xs text-muted-foreground">{label}</span><div className="flex min-w-0 items-center gap-2">
    ...
    <button type="button" className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full hover:bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring motion-safe:transition-colors" aria-label={`Copy ${label.toLowerCase()} address`} onClick={async () => { try { await navigator.clipboard.writeText(value); setCopied(true); setFailed(false); } catch { setFailed(true); } }}><Copy className="size-4" aria-hidden="true" /></button>
...
export function OnboardingJourney({ demo = false, executionEnabled = false }: { demo?: boolean; executionEnabled?: boolean }) {
  ...
  return <main id="main" className="page-width pb-12">
    <div className="flex flex-wrap items-center justify-between gap-3 py-6 text-xs text-muted-foreground sm:py-8"><Link href="/" className="inline-flex min-h-11 items-center rounded-full px-3 hover:bg-secondary/70 hover:text-foreground motion-safe:transition-colors">← Back to First Bite</Link><span className="glass-panel rounded-full px-4 py-2.5 font-medium text-foreground">{demo ? 'Walkthrough · no wallet or funds used' : controller.executionEnabled ? 'Sponsored registration · by invitation' : 'Wallet approvals are paused'}</span></div>
    <div className="grid items-start gap-8 pb-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)] lg:gap-10">
      <section aria-labelledby="journey-title" className="glass-panel min-w-0 rounded-3xl p-5 sm:p-8 lg:p-10">
        <ol aria-label="Your onboarding progress" className="mb-9 flex flex-wrap gap-2 border-b border-border/70 pb-6 text-xs">{chapters.map((label, i) => <li key={label} aria-current={chapter === i ? 'step' : undefined} className={`inline-flex items-center gap-2 rounded-full px-2.5 py-2 ${chapter === i ? 'bg-primary font-semibold text-primary-foreground' : i < chapter ? 'bg-pass/65 text-foreground' : 'text-muted-foreground'}`}><span className="font-mono text-xs">{i < chapter ? '✓' : `0${i + 1}`}</span>{label}</li>)}</ol>
        {state.restoring ? <div role="status" className="min-h-72"><p className="text-sm text-muted-foreground">Checking for your invitation…</p><div aria-hidden="true" className="mt-6 space-y-4"><div className="h-10 w-3/4 rounded bg-secondary" /><div className="h-5 w-full rounded bg-secondary" /><div className="h-12 rounded bg-secondary" /></div></div> : <>
          ...
          {state.offline && <p role="status" className="mt-5 flex gap-2 rounded-2xl border border-input/70 bg-background/75 p-4 text-sm"><WifiOff className="size-5 shrink-0" aria-hidden="true" />You’re offline. Reconnect to continue; an approved registration may still be processing.</p>}
          {state.error && <div role="alert" className="mt-5 rounded-2xl border border-input/70 bg-secondary/80 p-4 text-sm leading-6"><p>{state.error}</p><div className="mt-2 flex flex-wrap gap-3"><button type="button" disabled={busy} className="min-h-11 font-semibold underline underline-offset-4" onClick={() => void controller.refresh()}>Check saved progress</button>{state.step !== 'invite' && <button type="button" disabled={busy} className="min-h-11 font-semibold underline underline-offset-4" onClick={() => controller.invitationStep()}>Enter invitation again</button>}</div></div>}
          ...
          {state.step === 'wallet' && <div className="mt-6 space-y-6">
            ...
            {state.session && <div className="rounded-2xl border border-border/70 bg-background/60 p-5"><p className="mb-4 text-sm font-semibold">{state.session.campaign.name}</p><Address value={state.session.wallet} label="Assigned wallet" /></div>}
          ...
          {state.step === 'progress' && state.attempt && <div className="mt-6">
            ...
            {success ? <Result attempt={state.attempt} demo={demo} /> : <><ol className="my-7 space-y-5 text-sm">{['Approval received', 'Sent to Cookie', 'Confirmed by the network', 'Final ownership verified'].map((label, i) => { const reached = i === 0 ? !['prepared', 'expired'].includes(state.attempt!.status) : i === 1 ? ['submitted', 'confirmed'].includes(state.attempt!.status) : i === 2 ? state.attempt!.status === 'confirmed' : false; return <li key={label} className="flex items-center gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-full border border-input/70 bg-secondary/70">{reached ? <Check className="size-4" aria-hidden="true" /> : <span className="size-1 rounded-full bg-muted-foreground" />}</span>{label}{reached && <span className="sr-only"> — reached</span>}</li>; })}</ol><div className="flex flex-wrap gap-3"><Button variant="outline" disabled={busy || state.offline} onClick={() => void controller.refresh()}>Check progress</Button>{['failed', 'expired'].includes(state.attempt.status) && <Button disabled={busy} onClick={() => controller.freshName()}>Choose a fresh name</Button>}{state.attempt.status === 'manual_review' && <Button variant="outline" disabled={busy || !controller.executionEnabled} onClick={() => void controller.retry()}>Request another check</Button>}</div></>}
      ...
      <aside aria-label="Your First Bite pass" className="min-w-0 lg:sticky lg:top-8">
        <div className="relative isolate overflow-hidden rounded-3xl border border-border/60 bg-pass text-pass-foreground shadow-xl shadow-foreground/10">
          <div className="pass-grain pointer-events-none absolute inset-0" aria-hidden="true" />
          <div className="pointer-events-none absolute -top-16 -right-12 size-56 rounded-full border-[32px] border-background/15" aria-hidden="true" />
          <div className="relative p-7 sm:p-9">
            <div className="flex items-center justify-between gap-3 text-xs uppercase tracking-widest"><span className="font-mono">First Bite</span><BiteSymbol className="size-10" /></div>
            <p className="mt-12 text-xs uppercase tracking-widest">{success ? demo ? 'Example name' : 'Welcome to Cookie' : 'A place for your name'}</p>
            <p className="mt-4 break-all font-serif text-4xl leading-tight sm:text-5xl">{name}.cook</p>
            <p className="mt-10 text-sm">{state.session?.campaign.name ?? 'One invitation. A fresh start.'}</p>
          </div>
          <div className="relative flex items-center gap-3 border-t border-dashed border-current/30 bg-background/15 p-7 text-sm sm:px-9">{success ? <CheckCircle2 className="size-5 shrink-0" aria-hidden="true" /> : <Ticket className="size-5 shrink-0" aria-hidden="true" />}<span>{demo ? 'Example pass · no registration' : success ? 'Ownership verified' : 'Sponsored name + network fees'}</span></div>
        </div>
        <div className="mt-5 flex items-start gap-3 rounded-2xl border border-border/50 bg-background/50 p-5"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" /><p className="text-xs leading-6 text-muted-foreground">Your wallet stays yours. The pass covers your first step.</p></div>
      </aside>
    </div>
  </main>;
}
