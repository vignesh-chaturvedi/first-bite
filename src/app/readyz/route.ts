...
export async function GET(): Promise<Response> {
  try {
    ...
    return Response.json({ status: result.ready ? 'ready' : 'not_ready', scope: 'foundation', relayEnabled: false }, {
      status: result.ready ? 200 : 503,
      headers: { 'Cache-Control': 'no-store' },
    });
  } catch {
    logger('error', 'runtime.config_invalid', { failure: 'configuration' });
    return Response.json({ status: 'not_ready', scope: 'foundation', relayEnabled: false }, {
