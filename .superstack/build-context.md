...
- Architecture: Next.js web/API + PostgreSQL ledger + persistent Node worker,
  integrating the existing Cookie registry. Phase 0 currently contains only a
  TypeScript integration/proof harness and local wallet diagnostic.
- Phase: 00 local checkpoint passed; full feasibility gate remains open.
...
- Starter direction: use the official `create-solana-dapp`/Next.js frontend
  patterns in Phase1, adapted to the already approved stack; no starter cloned
  in this feasibility phase, no custom Anchor program generated.
